...
(function ($, window, document) {
	...
	******************************************************************************/
	var defaults = {
		...
		zoom_MAX: 400,								//Maximum limit for zooming (in percentage)
			...
			if (delta > 0) {
				...
				if (self.rA != self.zoom_min) {
					if (self.zoom_single){
						...
						self.rA /= delta > -1 ? 1 + (.3 * -delta) : 1.3;
						// self.rA = self.rA < self.rF ? self.rF : self.rA;
						self.rA = self.rA < self.zoom_min ?self.zoom_min : self.rA;
					}
					self._wheel = true;
					self.cFlag._zo = true;
					clearTimeout(self.ani_timer);
		...
		***********************************************************************************************************************/
		Animate: function () {
			...
			//Case 2: Normal Zoom
				...
				self._sc += (self.rA - self._sc) / (self.ani_smooth/(self._onfocus? self.reduction : .66));
			}
